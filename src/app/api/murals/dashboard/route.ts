  ...
  try {
    ...
    const sql = `
      WITH base_photos AS (
        ...
        FROM mural_photos p
        JOIN mural_events e ON e.id = p.event_id
      ...
      base_phrases AS (
        ...
        JOIN base_photos bp ON bp.photo_id = ph.photo_id
      ...
      SELECT
        ...
        -- regions breakdown (based on photos scope)
        (SELECT COALESCE(jsonb_object_agg(k, v), '{}'::jsonb)
        FROM (
          SELECT COALESCE(bp.region_name, 'Sin región') AS k, COUNT(*)::int AS v
          FROM base_phrases ph
          JOIN base_photos bp ON bp.photo_id = ph.photo_id
          GROUP BY COALESCE(bp.region_name, 'Sin región')
          ORDER BY COUNT(*) DESC
        ) t
        ) AS regions,

        -- events breakdown (based on photos scope)
        (SELECT COALESCE(jsonb_object_agg(k, v), '{}'::jsonb)
        FROM (
          SELECT
            (COALESCE(bp.name_event,'Sin evento') || ' — ' || COALESCE(bp.date_event::text,'')) AS k,
            COUNT(*)::int AS v
          FROM base_phrases ph
          JOIN base_photos bp ON bp.photo_id = ph.photo_id
          GROUP BY (COALESCE(bp.name_event,'Sin evento') || ' — ' || COALESCE(bp.date_event::text,''))
          ORDER BY COUNT(*) DESC
        ) t
        ) AS events,

        -- top phrases (based on extracted phrases scope)
