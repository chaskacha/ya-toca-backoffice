...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!search) {
      ...
      const countSql = `SELECT COUNT(*)::int AS total FROM t_topics s
            ${keywords.length > 0 ? `
              JOIN t_topic_keywords tk ON tk.topic_id = s.id
        ...
        SELECT DISTINCT ON (s.id)
          ...
          COALESCE((
            SELECT jsonb_agg(jsonb_build_object(
              ...
              FROM t_topic_keywords tk
              JOIN t_keywords k ON k.id = tk.keyword_id
             WHERE tk.topic_id = s.id
          ), '[]'::jsonb) AS keywords
        FROM t_topics s
        ${keywords.length > 0 ? `
          JOIN t_topic_keywords tk ON tk.topic_id = s.id
      ...
      const countSql = `
      WITH latest_emb AS (
        SELECT DISTINCT ON (topic_id) topic_id, vector
        FROM t_topic_embeddings
        ORDER BY topic_id, created_at DESC
      )
      SELECT COUNT(*)::int AS total
      FROM t_topics s
      LEFT JOIN latest_emb le ON le.topic_id = s.id
      WHERE (
        unaccent(lower(s.name)) ILIKE '%' || unaccent($1) || '%'
        OR unaccent(lower(s.description)) ILIKE '%' || unaccent($1) || '%'
...
`;
  ...
  CASE
    WHEN unaccent(lower(s.name)) ILIKE '%' || unaccent($4) || '%'
      OR unaccent(lower(s.description)) ILIKE '%' || unaccent($4) || '%'
    THEN 1 ELSE 0
  END AS lex_hit,

  GREATEST(
    similarity(unaccent(lower(s.name)), unaccent($4)),
    similarity(unaccent(lower(s.description)), unaccent($4))
  ) AS lex_score,
...
LEFT JOIN latest_emb le ON le.topic_id = s.id
WHERE (
      unaccent(lower(s.name)) ILIKE '%' || unaccent($4) || '%'
   OR unaccent(lower(s.description)) ILIKE '%' || unaccent($4) || '%'
